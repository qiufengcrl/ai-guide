...
function resolveActingUserId(ctx, explicit) {
...
async function readXhsCookieUpdatedAt(ctx, userId, cookie) {
  const stamps = [];
  try {
    stamps.push(parseTimestamp(await ctx.settings.get('xhs_cookie_updated_at')));
  } catch {
  ...
  if (userId != null) {
    try {
      ...
      stamps.push(parseTimestamp(rows[0]?.xhs_cookie_updated_at));
    } catch {
  ...
  if (fp) {
    try {
      ...
      stamps.push(parseTimestamp(rows[0]?.updated_at));
    } catch {
      // table may not exist yet
    }
  }
...
async function writeXhsCookieUpdatedAt(ctx, userId, cookie, at = Date.now()) {
  ...
  if (fp) {
    try {
      await ctx.db.exec(
        'INSERT INTO xhs_cookie_clock (fp, user_id, updated_at) VALUES (?, ?, ?) ON CONFLICT(fp) DO UPDATE SET updated_at = excluded.updated_at, user_id = COALESCE(excluded.user_id, xhs_cookie_clock.user_id)',
        fp,
        userId,
        stamp,
      );
